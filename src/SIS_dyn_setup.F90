...
subroutine direct_copy_to_EVPT(EVPT, CS, dt_slow, G, ci, ui, vi, mice,  &
                        fxat, fyat, pres_mice, diag_val, del_sh_min_pr, &
                        ui_min_trunc, ui_max_trunc, vi_min_trunc, vi_max_trunc, &
                        mi_u, f2dt_u, I1_f2dt2_u, PFu, mi_v, f2dt_v, I1_f2dt2_v, PFv, &
                        azon, bzon, czon, dzon, amer, bmer, cmer, dmer, &
  ...
  type(SIS_C_dyn_CS),   intent(in)     :: CS    !< The control structure for this module
  type(SIS_hor_grid_type),     intent(in)    :: G
  real, intent(in) :: dt_slow
  
  real, dimension(SZI_(G),SZJ_(G)),  intent(in   ) :: ci  !< Sea ice concentration [nondim]
  real, dimension(SZI_(G),SZJ_(G)),  intent(in   ) :: mice  !< Mass per unit ocean area of sea ice [R Z ~> kg m-2]
  real, dimension(SZIB_(G),SZJ_(G)),  intent(in   ) :: ui    !< Zonal ice velocity [L T-1 ~> m s-1]
  real, dimension(SZI_(G),SZJB_(G)),  intent(in   ) :: vi    !< Meridional ice velocity [L T-1 ~> m s-1]
  ...
  EVPT%SIS_C_dyn_CSp = CS

  EVPT%dt_slow = dt_slow
  ...
  EVPT%pres_mice(:,:)     = pres_mice(:,:)
  EVPT%diag_val(:,:)      = diag_val(:,:)
  EVPT%del_sh_min_pr(:,:) = del_sh_min_pr(:,:)

  EVPT%ui_min_trunc(:,:) = ui_min_trunc(:,:)
  EVPT%ui_max_trunc(:,:) = ui_max_trunc(:,:)
  EVPT%mi_u(:,:)         = mi_u(:,:)
  EVPT%f2dt_u(:,:)       = f2dt_u(:,:)
  EVPT%PFu(:,:)          = PFu(:,:)
  EVPT%I1_f2dt2_u(:,:)   = I1_f2dt2_u(:,:)
  
  EVPT%vi_min_trunc(:,:) = vi_min_trunc(:,:)
  EVPT%vi_max_trunc(:,:) = vi_max_trunc(:,:)
  EVPT%mi_v(:,:)         = mi_v(:,:)
  EVPT%f2dt_v(:,:)       = f2dt_v(:,:)
  EVPT%PFv(:,:)          = PFv(:,:)
  EVPT%I1_f2dt2_v(:,:)   = I1_f2dt2_v(:,:)
  ...
  EVPT%mi_ratio_A_q(:,:) = mi_ratio_A_q(:,:)

end subroutine direct_copy_to_EVPT
...
end subroutine  direct_copy_from_updated_EVPT
