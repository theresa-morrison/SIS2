...
use MOM_SIS_dyn_evp, only :  direct_copy_to_EVPT, direct_copy_from_EVPT, SIS_C_EVP_state
...
#include "version_variable.h"
  ...
  call get_param(param_file, mdl, "DO_EMBEDDED", CS%do_embedded, &
               "If true, do rheology steps in MOM6"//&
               ".", default=.false.)
...
subroutine SIS_C_dynamics(ci, mis, mice, ui, vi, uo, vo, fxat, fyat, &
                          sea_lev, fxoc, fyoc, dt_slow, G, US, CS, evp_ptr)

  type(SIS_hor_grid_type),           intent(inout) :: G   !< The horizontal grid type
  real, dimension(SZI_(G),SZJ_(G)),  intent(inout) :: ci  !< Sea ice concentration [nondim]
  real, dimension(SZI_(G),SZJ_(G)),  intent(inout) :: mis   !< Mass per unit ocean area of sea ice,
                                                            !! snow and melt pond water [R Z ~> kg m-2]
  real, dimension(SZI_(G),SZJ_(G)),  intent(inout) :: mice  !< Mass per unit ocean area of sea ice [R Z ~> kg m-2]
  ...
  real, dimension(SZIB_(G),SZJ_(G)), intent(inout) :: fxat  !< Zonal air stress on ice [R Z L T-2 ~> Pa]
  real, dimension(SZI_(G),SZJB_(G)), intent(inout) :: fyat  !< Meridional air stress on ice [R Z L T-2 ~> Pa]
  ...
  real,                              intent(inout) :: dt_slow !< The amount of time over which the ice
                                                            !! dynamics are to be advanced [T ~> s].
  type(unit_scale_type),             intent(in)    :: US    !< A structure with unit conversion factors
  type(SIS_C_dyn_CS),                pointer       :: CS    !< The control structure for this module
  type(SIS_C_EVP_state),   optional, pointer       :: evp_ptr !<
  ...
  if (.not.CS%do_embedded .or. CS%embedded_setup) then
  ...
  !$OMP end parallel
    ...
    dt_cumulative = 0.0
  endif !(.not.CS%do_embedded .or. CS%embedded_setup) then

  if (.not.CS%do_embedded) then 
  ...
  !$OMP                                  m_vio_explicit,b_vel0,vio_pred,vio_C)
    ...
    if (CS%embedded_setup) then ! first
     call direct_copy_to_EVPT(evp_ptr, CS, dt_slow, G, ci, ui, vi, mice,  &
                        ...
                        mi_ratio_A_q)
    elseif (CS%embedded_finish) then! second
     call direct_copy_from_EVPT(evp_ptr, CS, dt_slow, G, ci, ui, vi, mice,  &
                        fxat, fyat, pres_mice, diag_val, del_sh_min_pr, &
                        ui_min_trunc, ui_max_trunc, vi_min_trunc, vi_max_trunc, &
                        mi_u, f2dt_u, I1_f2dt2_u, PFu, mi_v, f2dt_v, I1_f2dt2_v, PFv, &
                        azon, bzon, czon, dzon, amer, bmer, cmer, dmer, &
  ...
  if (.not.CS%do_embedded .or. CS%embedded_finish) then
  ...
  endif ! (.not.CS%do_embedded .or. CS%embedded_finish) then
